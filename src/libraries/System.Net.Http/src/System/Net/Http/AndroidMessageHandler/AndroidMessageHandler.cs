...
public class AndroidMessageHandler : HttpMessageHandler
{
    ...
    private Task<HttpResponseMessage> MakeRequest(HttpRequestMessage request, CancellationToken cancellationToken)
        => Task.Run(() => Interop.Http.MakeRequest(request, handler: this, cancellationToken));
